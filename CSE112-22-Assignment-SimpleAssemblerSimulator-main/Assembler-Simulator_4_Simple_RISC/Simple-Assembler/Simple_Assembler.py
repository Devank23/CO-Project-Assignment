...
File_main_open = open('C:/Python/errorsassembler.txt','w')
Memory_Address=[]
i=0                 #line counter
...
class Err:
    ...
    def Typos_Error_Check(Instruction_Value): # Gives the type of instruction, input arg is in form of array of strings of each line
        ...
        if(z3[0]=="label"):
            x= Err.Typos_Error_Check(Instruction_Value[1: len(Err.Typos_Error_Check)])
            return x
    ...
    def SYNTAX_ERROR(Instruction_Value): # Checks the syntax of the given line, and returns true if any error is found.
        ...
        if(z2[0]=="label"):
            a= Err.SYNTAX_ERROR(Instruction_Value[1 : len(Err.SYNTAX_ERROR)])
            return a
        ...
        if(Instruction_Type=="F"):
            ...
            if(len(Instruction_Value)!=4):
                ...
                File_main_open.write("[Error Detected] : Typos in register name\n")
                return(E_Flag_2)
    ...
    def WronG_Var_Declaration(line):  # Returns true if undefined variable is used
        ...
        if Instruction_Type=="label":
            a = Err.WronG_Var_Declaration(line[1: len(Err.WronG_Var_Declaration)])
            return a
    ...
    def Wrong_Label(line): #returns true if undefined label is used
        Instruction_Type=FInd_Type(line)
        if Instruction_Type=="label":
            a = Err.Wrong_Label(line[1:len(Err.Wrong_Label)])
            return a
    ...
    def Imm_Val_Error(line): # Checks if immediate value is invalid.
        Instruction_Type=FInd_Type(line)
        if Instruction_Type=="label":
            a = Err.FLag_Reg_Error(line[1:len(Err.FLag_Reg_Error)])
            return a
    ...
    def ifValid_Var(line): # check if the given variable is valid.
        Instruction_Type=FInd_Type(line)
        if Instruction_Type=="label":
            a = Err.ifValid_Var(line[1: len(Err.ifValid_Var)])
            return a
    ...
    def Immediatie_Error_Check(line): # Checks the validity of variable from Type B.
        Instruction_Type=FInd_Type(line)
        if Instruction_Type=="label":
            a = Err.Variable_Label_Check(line[1:len(Err.Variable_Label_Check)])
            return a
        if Instruction_Type=="B":
    ...
    def Label_Vriable_Check(line): # Checks the validity of variable from Type C.
        Instruction_Type=FInd_Type(line)
        if Instruction_Type=="label":
            a = Err.Label_Vriable_Check(line[1:len(Err.Label_Vriable_Check)])
            return a
...
with open("C:/Python/errors.txt","r") as f:
...
if(lenght)>256:
    ...
    File_main_open.write("Error Detected : The assembler can only write < or = 256 lines.")
...
for instruction in range(len(Updated_List_of_Data)):
    ...
    if type=="F":
        ...
        if(instruction!=len(Updated_List_of_Data)-1):
            E_Flag_2=True
            print(errors[10])
            File_main_open.write(errors[10])
            sys.exit()
